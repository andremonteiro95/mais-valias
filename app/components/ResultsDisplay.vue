...
const fmtPct = (v: number) =>
  new Intl.NumberFormat('pt-PT', { style: 'percent', minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(v)
    ...
    <UAlert v-if="isLoss" color="info" icon="i-lucide-trending-down" title="Menos-valia">
      <template #description>
        <span class="flex items-center gap-1.5">
          O valor de venda é inferior ao custo ajustado — não há mais-valia tributável. A menos-valia pode ser deduzida a mais-valias de outros imóveis no mesmo ano.
          <UTooltip text="Art. 55.º CIRS — o saldo negativo pode ser reportado e deduzido a mais-valias dos cinco anos seguintes. Apenas aplicável a residentes.">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between py-2 border-b border-default">
          <div class="flex items-center gap-2">
            ...
            <UTooltip text="MV = VR − (VA × Coef.) − Encargos valorização − Despesas. Art. 44.º a 51.º CIRS">
        ...
        <div class="flex items-center justify-between py-2 border-b border-default">
          <div class="flex items-center gap-2">
            ...
            <UTooltip text="Para residentes em Portugal, apenas 50% do saldo é tributado às taxas progressivas do IRS. Art. 43.º, n.º 2, al. b) CIRS">
              <UIcon name="i-lucide-info" class="text-muted size-3.5 cursor-help" />
            </UTooltip>
          </div>
        ...
        <div v-if="reinvestimentoParcial" class="flex items-center justify-between py-2 border-b border-default">
          ...
          <UBadge
            :color="percentagemReinvestimento >= 1 ? 'success' : percentagemReinvestimento > 0 ? 'warning' : 'neutral'"
            variant="soft"
            size="lg"
          >
        ...
        <div v-if="isFullExemption" class="pt-1">
          <UAlert color="success" icon="i-lucide-shield-check" title="Isenção total">
            <template #description>
              <span class="flex items-center gap-1.5">
                Reinvestindo a totalidade do valor necessário, a mais-valia fica excluída de tributação.
                <UTooltip text="Art. 10.º, n.º 5 CIRS — exclusão condicionada a: reinvestimento em nova HPP, prazo de 24 meses antes a 36 meses após a venda, domicílio fiscal no imóvel alienado nos 12 meses anteriores à transmissão (DL n.º 57/2024) e declaração de intenção no Anexo G.">
                  <UIcon name="i-lucide-info" class="size-3.5 cursor-help shrink-0" />
                </UTooltip>
              </span>
            </template>
        ...
        <template v-if="reinvestimentoParcial && valorReinvestido > 0">
          <div class="flex items-center justify-between py-2 border-b border-default">
            <div class="flex items-center gap-2">
              ...
              <UTooltip text="MV isenta = MV total × (reinvestido / valor a reinvestir). Art. 10.º, n.º 9 CIRS">
                <UIcon name="i-lucide-info" class="text-muted size-3.5 cursor-help" />
              </UTooltip>
            </div>
          ...
          <div class="flex items-center justify-between py-2">
            <div class="flex items-center gap-2">
              <span class="text-sm text-muted">Mais-valia tributável após reinvestimento parcial (50%)</span>
              <UTooltip text="(MV total − MV isenta) × 50%. Art. 10.º, n.º 9 CIRS">
                <UIcon name="i-lucide-info" class="text-muted size-3.5 cursor-help" />
              </UTooltip>
            </div>
            <UBadge color="error" variant="soft" size="lg">{{ fmt(maisValiaTributavelParcial) }}</UBadge>
          </div>
        </template>
      </div>
